...
java -cp "/opt/hazelcast/*:/opt/hazelcast/lib/*:/opt/hazelcast/apclib/*:"  "-Duser.timezone=GMT -Djava.net.preferIPv4Stack=true -Dcom.sun.management.jmxremote -Dhazelcast.jmx=true " $JAVA_OPTS com.actiance.platform.sfab.cis.cache.server.AlcatrazHazelcastServer >> /opt/hazelcast/hazelcast.log 2>&1 &
PID="$!"
...
trap - SIGTERM SIGINT
wait ${PID}
tail -f /opt/hazelcast/hazelcast.log
